...
import { mapPostToFeedView, POST_TYPE_LABELS, type FeedPostView } from '@/lib/mappers/posts'
...
import { FeedFilterSheet, type FeedPostTypeFilter } from '@/components/feed/feed-filter-sheet'
...
type FeedScope = 'all' | 'connections' | 'my_posts'
...
export default function FeedPage() {
  ...
  const [deletePostTarget, setDeletePostTarget] = useState<FeedPostView | null>(null)
  const [deletingPost, setDeletingPost] = useState(false)

  const isAdmin = user?.platform_role === 'SUPER_ADMIN'
  const canDeletePost = useCallback(
    (post: FeedPostView) => {
      if (!user?.id) return false
      if (isAdmin) return true
      return feedScope === 'my_posts' && post.author.id === user.id
    },
    [feedScope, isAdmin, user?.id]
  )
  ...
  const handleConfirmDeletePost = async () => {
    ...
    try {
      await postsAPI.deletePost(deletePostTarget.id)
      setFeedPosts((prev) => prev.filter((p) => p.id !== deletePostTarget.id))
  ...
  const loadFeed = useCallback(async (page: number, append: boolean) => {
    ...
    try {
      const typeParam = postTypeFilter === 'all' ? undefined : postTypeFilter
      const postsRes = await postsAPI.getFeed(page, FEED_PAGE_SIZE, {
        feedScope,
        postType: typeParam,
      })
      const mapped = (postsRes.data.posts || []).map(mapPostToFeedView)
      setHasMoreFeed(Boolean(postsRes.data.has_more))
      setFeedPosts((prev) => (append ? [...prev, ...mapped] : mapped))
      setFeedPage(page)
    } catch {
  ...
  const loadSidebar = useCallback(async () => {
    try {
      const recRes = await usersAPI.getDirectory()
      setTrendingPeople(recRes.data.users || [])
    } catch {
      setTrendingPeople([])
    }
  }, [])
  ...
  useEffect(() => { void loadFeed(1, false) }, [loadFeed])
      ...
      setFeedPosts((prev) =>
        prev.map((p) => {
          ...
          return {
            ...p,
            poll: {
              options: (details.options || []).map((o: { id: string; text: string; vote_count?: number; percentage?: number }) => ({
                id: o.id,
                text: o.text,
  ...
  const handleSharePost = async (postId: string) => {
    ...
    if (result === 'modal') {
      setSharePayload(getPostSharePayload(postId))
          ...
          {mainTab === 'feed' && (
            ...
            >
              <div className="w-full max-w-3xl mx-auto space-y-2">
                  {/* Feed scope tabs + actions */}
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-border/40 pb-2">
                    <div className="flex items-center gap-1 overflow-x-auto">
                      {([
                        { id: 'all' as FeedScope, label: 'All Posts' },
                        { id: 'connections' as FeedScope, label: 'My Connections' },
                        { id: 'my_posts' as FeedScope, label: 'My Posts' },
                      ]).map((tab) => (
                        <button
                          key={tab.id}
                          type="button"
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <Button
                        size="sm"
                        className="h-8 gap-1.5 glow-primary"
                  ...
                  <div className="space-y-2">
                    {feedLoading && feedPosts.length === 0 && (
                      <div className="glass-card p-6 flex items-center justify-center gap-3 text-muted-foreground">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <ReactionButtons
                              ...
                              onShareClick={() => void handleSharePost(post.id)}
                            />
              ...
              <FeedFilterSheet
                open={feedFiltersOpen}
                onOpenChange={setFeedFiltersOpen}
              ...
              <Dialog open={!!deletePostTarget} onOpenChange={(open) => !open && !deletingPost && setDeletePostTarget(null)}>
                    ...
                    <div className="glass-card relative overflow-hidden p-8 md:p-10">
                      ...
                      <div className="relative z-10 max-w-2xl space-y-3">
                        ...
                        <p className="text-sm text-muted-foreground max-w-lg">
                          Select one of your repositories, describe the issue you're stuck on, and post it to the RConnectX collaboration marketplace. Fellow developers will claim and help you resolve it.
                        </p>
                        {!isConnected && (

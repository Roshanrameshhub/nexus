...
export default function PublicProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleShare = async () => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: `${profile?.name} on RConnectX`, url })
        return
      }
    } catch {
      /* copy fallback */
        ...
        <div className="glass-card p-8 relative">
          ...
          <div className="text-center">
            ...
            <h1 className="text-2xl font-bold flex flex-wrap items-center justify-center gap-2">
              {profile.name}
              <VerifiedBadge verified={profile.is_verified} variant="badge" label={getVerifiedBadgeLabel(profile.role)} />
            </h1>

...
import { useDirectory, useUserSearch } from '@/lib/hooks/api/use-users'
...
export default function DiscoverPage() {
  ...
  const { data: directoryUsers, isLoading: loadingDirectory } = useDirectory()
  ...
  const list = query.length >= 1 ? searchResults : directoryUsers
  const loading = query.length >= 1 ? searching : loadingDirectory
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <section>
          <h2 className="text-lg font-semibold mb-4">
            {query ? 'Search results' : 'All members'}
          </h2>

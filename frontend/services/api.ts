...
if (typeof window !== 'undefined' && !process.env.NEXT_PUBLIC_API_URL) {
  console.warn(
    '[RConnectX] NEXT_PUBLIC_API_URL is not set. Using',
    API_BASE_URL,
...
export const dashboardAPI = {
  ...
  trackAnnouncementView: (id: string) => api.post(`/dashboard/announcements/${id}/view`),
  trackAnnouncementClick: (id: string) => api.post(`/dashboard/announcements/${id}/click`),
  dismissAnnouncement: (id: string) => api.post(`/dashboard/announcements/${id}/dismiss`),
}
...
export const usersAPI = {
  ...
  getRecommendations: (roles?: string[], country?: string, limit = 5) => {
    ...
    return api.get(`/users/recommendations?${qs}`)
  },
  getDirectory: (roles?: string[], country?: string) => {
    const params = new URLSearchParams()
    if (roles?.length) {
      roles.forEach((r) => params.append('role', r))
    }
    ...
    const qs = params.toString()
    return api.get(`/users/directory${qs ? `?${qs}` : ''}`)
  },
  ...
  presence: (ids: string[]) => {
    if (!ids.length) return Promise.resolve({ data: { presence: [] } })
    return api.get(`/users/presence?ids=${ids.map(encodeURIComponent).join(',')}`)
  },
}
...
export const postsAPI = {
  getFeed: (
    ...
    options?: {
      ...
      postType?: string
      feedScope?: 'all' | 'connections' | 'my_posts'
    }
  ) => {
    const params = new URLSearchParams({
      page: String(page),
    ...
    if (options?.ecosystemCategory && options.ecosystemCategory !== 'all') {
      params.set('ecosystem_category', options.ecosystemCategory)
    }
    if (options?.postType && options.postType !== 'all') {
      params.set('post_type', options.postType)
    }
    if (options?.feedScope && options.feedScope !== 'all') {
      params.set('feed_scope', options.feedScope)
    }
  ...
  votePoll: (id: string, optionId: string) =>
    api.post(`/posts/${id}/poll-vote`, { option_id: optionId }),
...
export const notificationsAPI = {
  ...
  registerPushToken: (data: { platform: string; token: string; subscription_json?: string }) =>
    api.post('/notifications/push-token', data),
  unregisterPushToken: (data: { platform: string; token: string }) =>
...
export const adminAPI = {
  ...
  streaks: (params?: {
    sort_by?: 'current' | 'longest' | 'monthly' | 'all_time'
    role?: string
    country?: string
    state?: string
    city?: string
    ...
    if (params?.limit) search.set('limit', String(params.limit))
    const qs = search.toString()
    return api.get(`/admin/streaks${qs ? `?${qs}` : ''}`)
  },
  ...
  warnUser: (id: string, message?: string) =>
    api.post(`/admin/users/${id}/warn`, { message }),
  deleteUser: (id: string, confirm_email: string) =>
    api.delete(`/admin/users/${id}`, { data: { confirm_email } }),
  ...
  broadcastAnnouncement: (data: Record<string, unknown>) => api.post('/admin/broadcast/announcement', data),
  broadcastAdminPost: (data: Record<string, unknown>) => api.post('/admin/broadcast/admin-post', data),
  broadcastNotification: (data: Record<string, unknown>) => api.post('/admin/broadcast/notification', data),
  contentPosts: (tab: string) => api.get(`/admin/content/posts?tab=${encodeURIComponent(tab)}`),
  pinnedPosts: () => api.get('/admin/pinned'),
  pinPost: (post_id: string, pin_order?: number, expiry_days?: number) =>
    api.post('/admin/pinned', { post_id, pin_order, expiry_days }),
  ...
  reports: (params?: { status?: string; type?: string }) => {
    const search = new URLSearchParams()
  ...
  moderateReport: (data: {
    ...
    action: string
    note?: string
  }) => api.post('/admin/reports/action', data),

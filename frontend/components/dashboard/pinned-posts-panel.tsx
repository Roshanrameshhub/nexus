...
export function PinnedPostsPanel({ open, onOpenChange, posts, onPostsChange }: PinnedPostsPanelProps) {
      ...
      <SheetContent side="right" className="w-full sm:max-w-xl p-0 flex flex-col">
        <SheetHeader className="px-6 py-5 border-b border-border shrink-0">
          <div className="flex items-center gap-2">
            ...
            <SheetTitle>Pinned Posts</SheetTitle>
          </div>
          <SheetDescription>Curated posts highlighted by the RConnectX team.</SheetDescription>
        </SheetHeader>
        <ScrollArea className="flex-1 px-6 py-4">
          <div className="space-y-3 pb-6">
            {posts.length === 0 && (
              <p className="text-sm text-muted-foreground text-center py-12">No pinned posts right now.</p>
            )}
            {posts.map((post) => (

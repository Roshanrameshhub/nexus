...
from app.utils.location import location_similarity_score
...
class RecommendationService:
    ...
    def calculate_match_score_and_factors(self, current_user: User, target_user: User) -> Tuple[int, List[str]]:
        ...
        if r1 == UserRole.founder and r2 == UserRole.investor:
            ...
            loc_score, loc_factors = location_similarity_score(current_user, target_user)
            if loc_score:
                score += loc_score
                for f in loc_factors:
                    factors.append(f"✓ {f}")
    ...
    def score_relationship_recommendation(
        ...
        if (
            (current_user.country or getattr(current_user, "city", None) or getattr(current_user, "state", None))
            and (target_user.country or getattr(target_user, "city", None) or getattr(target_user, "state", None))
        ):
            loc_score, loc_factors = location_similarity_score(current_user, target_user)
            score += loc_score

...
type DocumentType = 'college_id' | 'company_id'
...
const DOCUMENT_OPTIONS: { value: DocumentType; label: string; description: string }[] = [
...
export function VerificationSection() {
  ...
  const [documentType, setDocumentType] = useState<DocumentType>('college_id')
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground max-w-xl">
            Optional identity verification helps build trust on Nexus. Upload a college or company ID for admin review.
          </p>
        </div>
        <span
          className={cn(
      ...
      {status === 'verified' && (
        <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/5 p-4 flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-foreground">You are a verified member</p>
            <p className="text-sm text-muted-foreground mt-0.5">
              Your identity has been confirmed by the Nexus team.
            </p>
          </div>
        </div>
      )}
      ...
      {status === 'pending' && state?.latest_request && (
        <div className="rounded-xl border border-amber-500/25 bg-amber-500/5 p-4">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            ...
            {' · '}
            {state.latest_request.document_type === 'college_id' ? 'College ID' : 'Company ID'}
          </p>
        </div>
      )}
